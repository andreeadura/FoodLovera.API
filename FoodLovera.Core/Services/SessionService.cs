...
public sealed class SessionService : ISessionService
{
    ...
    public async Task<CreateSessionResponse> CreateAsync(CreateSessionRequest request, CancellationToken ct)
    {
        ...
        // 3) Create session (Id generated by DB)
        ...
        if (!useAllCategories)
        {
            // Important: do NOT set SessionId here (unknown before save).
    ...
    public async Task<NextResponse> NextAsync(int sessionId, NextRequest request, CancellationToken ct)
    {
        if (request is null) throw new ArgumentNullException(nameof(request));
    ...
    public async Task<LikeResponse> LikeAsync(int sessionId, int restaurantId, LikeRequest request, CancellationToken ct)
    {
        if (request is null) throw new ArgumentNullException(nameof(request));
        if (request.ParticipantId <= 0)
            throw new ArgumentException("ParticipantId is required.", nameof(request.ParticipantId));
    ...
    private async Task<List<WinnerResponse>> GetMatchWinnersAsync(int sessionId, CancellationToken ct)
    {
        var participantCount = await _participants.CountActiveAsync(sessionId, ct);
    ...
    private async Task<List<WinnerResponse>> GetTopLikesWinnersAsync(int sessionId, CancellationToken ct)
    {
